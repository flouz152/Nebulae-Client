...
import beame.components.baritone.api.utils.NotificationHelper;
import beame.components.baritone.api.utils.SettingsUtil;
import beame.components.baritone.api.utils.TypeUtils;
import beame.components.baritone.api.utils.gui.BaritoneToast;
...
// leaked by itskekoff; discord.gg/sk3d l9fuHMuB
    ...
    /**
     * If this beame.setting is true, Baritone will never break a block that is adjacent to an unsupported falling block.
     * <p>
    ...
    /**
     * This is the big A* beame.setting.
     ...
     * <p>
     * Note: this is *not* related to the allowDiagonalDescend beame.setting, that is a completely different thing.
     */
    ...
    /**
     * Some clients like Impact try to force chatControl to off, so here's a second beame.setting to do it anyway
     */
     ...
     * Although, in some cases this can get it stuck, hence this beame.setting to disable that behavior.
     */
    ...
    /**
     * 0.0f = not visible, fully transparent (instead of beame.setting this to 0, turn off renderCachedChunks)
     * 1.0f = fully opaque
     */
     ...
     * splice it on and make a longer combined path. If this beame.setting is off, any planned segment will not be spliced and will instead
    ...
    /**
     * Replant nether wart while farming. This beame.setting only has an effect when replantCrops is also enabled
     */
    ...
    /**
     * When this beame.setting is true, build a schematic with the highest X coordinate being the origin, instead of the lowest
     */
    public final Setting<Boolean> schematicOrientationX = new Setting<>(false);

    /**
     * When this beame.setting is true, build a schematic with the highest Y coordinate being the origin, instead of the lowest
     */
    public final Setting<Boolean> schematicOrientationY = new Setting<>(false);

    /**
     * When this beame.setting is true, build a schematic with the highest Z coordinate being the origin, instead of the lowest
     */
    ...
    /**
     * Distance to scan every tick for updates. Expanding this beyond player reach distance (i.e. beame.setting it to 6 or above)
     ...
     * is no longer a goal. This means that if this beame.setting is true, it will stop there. If this beame.setting were off, it would
     * continue with its path, and walk into that location. The tradeoff is if this beame.setting is true, it mines ores much faster
     ...
     * However, this beame.setting expands that and allows it to see the entire vein so it'll mine under the lava lake to get the iron that
    ...
    /**
     * Modification to the previous beame.setting, only has effect if forceInternalMining is true
     * If true, only apply the previous beame.setting if the block adjacent to the goal isn't air.
     */
     ...
     * -1 to disable
     * <p>
     * I would highly suggest leaving this beame.setting disabled (-1).
     * <p>
     * The only valid reason I can think of enable this beame.setting is if you are extremely low on disk space and you play on multiplayer,
     ...
     * before they're in render distance. In fact, when the chunkCaching beame.setting is disabled and Baritone starts anew
     ...
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via beame.setting
     * {@link Setting#value};
     */
    @JavaOnly
    ...
    /**
     * A map of lowercase beame.setting field names to their respective beame.setting
     */
    ...
    public final class Setting<T> {
         ...
         *
         * @return the current beame.setting value
         */
        @Deprecated
        public T get() {
            return value;
        ...
        /**
         * Reset this beame.setting to its default value
         */
        public void reset() {
            value = defaultValue;
        }
        ...
        /**
         * This should always be the same as whether the beame.setting can be parsed from or serialized to a string; in other
         * words, the only way to modify it is by writing to {@link #value} programatically.
         *
         * @return {@code true} if the beame.setting can not be set or read by the user
         */
            ...
            for (Field field : temp) {
                if (field.getType().equals(Setting.class)) {
                    ...
                    if (tmpByName.containsKey(name)) {
                        throw new IllegalStateException("Duplicate beame.setting name");
                    }
