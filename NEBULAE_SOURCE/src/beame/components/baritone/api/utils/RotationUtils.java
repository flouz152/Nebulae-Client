...
// leaked by itskekoff; discord.gg/sk3d OIKAr2GH
    ...
    public static Optional<Rotation> reachable(IPlayerContext ctx, BlockPos pos, double blockReachDistance, boolean wouldSneak) {
        if (BaritoneAPI.getSettings().remainWithExistingLookDirection.value && ctx.isLookingAt(pos)) {
             ...
             * is by beame.setting the desired pitch to the current pitch
             * beame.setting the desired pitch to the current pitch + 0.0001 means that we do have a desired pitch, it's
