...
// leaked by itskekoff; discord.gg/sk3d l4ZfsZrV
    ...
    public Dragging KeybindsDrag = Nebulae.getHandler().createDraggable("keybinds", 90, 95);
    float animatedHbinds = 16;
    float animatedWbinds = 80;
    private float animation = 0;

    public void render() {
        boolean isEnabled = !Nebulae.getHandler().getModuleList().getModules().stream().filter(module -> module.isState() && module.getBind() > 0).toList().isEmpty();
        ...
        for (Module m : Nebulae.getHandler().getModuleList().getModules()) {
        ...
        ClientHandler.drawSexyRect(x, y, animatedWbinds, 16, Nebulae.getHandler().getModuleList().hud.rounding.get(), true);


        if (hasBinds) {
            ClientHandler.drawSexyRect(x, y + 20, animatedWbinds, animatedHbinds, Nebulae.getHandler().getModuleList().hud.rounding.get(), false);
            ...
            for (Module m : Nebulae.getHandler().getModuleList().getModules()) {
                ...
                if (m.isState() && m.getBind() != 0) {
                    ...
                    f.drawString(b, x + animatedWbinds - 8 - f.getStringWidth(b), y + 21.5f + (bindCountV*11) + 2.5f + 3, ColorUtils.setAlpha(Nebulae.getHandler().themeManager.getColor(0), alpha));
