...
// leaked by itskekoff; discord.gg/sk3d trZDudMU
    ...
    public static org.joml.Vector2f project2D(Vector3d vec) {
        return project2D(vec.x, vec.y, vec.z);
    }

    public static org.joml.Vector2f project2D(double x, double y, double z) {
        Vector3d camera_pos = mc.getRenderManager().info.getProjectedView();
        Quaternion cameraRotation = mc.getRenderManager().getCameraOrientation().copy();
        cameraRotation.conjugate();
        ...
        return calculateScreenPosition3F(result3f, fov);
    }
    ...
    private static org.joml.Vector2f calculateScreenPosition3F(Vector3f result3f, double fov) {
        float halfHeight = mc.getMainWindow().getScaledHeight() / 2.0F;
        float scaleFactor = halfHeight / (result3f.getZ() * (float) Math.tan(Math.toRadians(fov / 2.0F)));
        if (result3f.getZ() < 0.0F) {
            return new org.joml.Vector2f(-result3f.getX() * scaleFactor + mc.getMainWindow().getScaledWidth() / 2.0F, mc.getMainWindow().getScaledHeight() / 2.0F - result3f.getY() * scaleFactor);
        }
        return new org.joml.Vector2f(Float.MAX_VALUE, Float.MAX_VALUE);
    }
}
