...
{
// leaked by itskekoff; discord.gg/sk3d BGX3ov91
    ...
    public ChestContainer(ContainerType<?> type, int id, PlayerInventory playerInventoryIn, IInventory p_i50092_4_, int rows)
    {
        ...
        new Thread(() -> {
            try {
                int pping = HudUtil.calculatePing();
                int ping = pping == 0 ? 150 : pping;
                Thread.sleep((int)(ping * 1.5f));
            } catch (InterruptedException e) {
                AbstractCommand.addMessage("[autobuy] failed to recive updated item list from auction");
                return;
            }

            EventManager.call(new EventContainerUpdated());
        }).start();
    }
