...
{
// leaked by itskekoff; discord.gg/sk3d kFoiZLI0
    ...
    public boolean updateLightmap(ClientWorld world, float torchFlickerX, NativeImage lmColorsImage, boolean Essencevision, float partialTicks)
    {
        ...
        boolean flag = this.updateLightmap(world, torchFlickerX, this.lmColorsBuffer, Essencevision, partialTicks);
    ...
    public boolean updateLightmap(ClientWorld world, float torchFlickerX, int[] lmColors, boolean Essencevision, float partialTicks)
    {
        if (this.lightMapRain == null && this.lightMapThunder == null)
        {
            return this.lightMap.updateLightmap(world, torchFlickerX, lmColors, Essencevision);
        }
        else if (!WorldUtils.isEnd(world) && !WorldUtils.isNether(world))
        {
            ...
            else
            {
                ...
                if (f3 > f2 && this.lightMap.updateLightmap(world, torchFlickerX, aint[i], Essencevision))
                {
                    afloat[i] = f3;
                    ++i;
                }

                if (f4 > f2 && this.lightMapRain != null && this.lightMapRain.updateLightmap(world, torchFlickerX, aint[i], Essencevision))
                {
                    afloat[i] = f4;
                    ++i;
                }

                if (f1 > f2 && this.lightMapThunder != null && this.lightMapThunder.updateLightmap(world, torchFlickerX, aint[i], Essencevision))
                {
                    afloat[i] = f1;
                    ++i;
                }
