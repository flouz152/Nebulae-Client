...
@AllArgsConstructor
public enum KeyboardUtil implements IMinecraft {
// leaked by itskekoff; discord.gg/sk3d ESP2V4mq
    /**
     * Printable keys.
     */
    ...
    public static String keyName(int keyCode) {
        return Arrays.stream(KeyboardUtil.values()).filter(key -> key.keyCode == keyCode).findFirst().orElse(KeyboardUtil.KEY_NONE).name;
    }

    public static int keyCode(String keyName) {
        return Arrays.stream(KeyboardUtil.values()).filter(key -> key.name.equalsIgnoreCase(keyName)).findFirst().orElse(KeyboardUtil.KEY_NONE).keyCode;
    }
