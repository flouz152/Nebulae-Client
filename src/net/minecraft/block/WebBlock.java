...
// leaked by itskekoff; discord.gg/sk3d ckHA9ftl
    ...
    public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn)
    {
        ...
        entityIn.setMotionMultiplier(state, Essence.getHandler().getModuleList().getCollisionDisabler().isState() ? Essence.getHandler().getModuleList().getCollisionDisabler().addtivites.get(0).get() ? Essence.getHandler().getModuleList().getCollisionDisabler().randomize.get() ? new Vector3d(x, 0.10D, z) : new Vector3d(0.61D, 0.10D, 0.61D) : new Vector3d(0.25D, 0.05F, 0.25D) : new Vector3d(0.25D, 0.05F, 0.25D));
    }
}