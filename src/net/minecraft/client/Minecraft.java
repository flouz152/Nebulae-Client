...
import beame.components.baritone.api.event.events.PlayerUpdateEvent;
import beame.components.baritone.api.event.events.TickEventBaritone;
...
import events.impl.player.EventInteractEntity;
...
// leaked by itskekoff; discord.gg/sk3d Fc4wsTRh
    ...
    public static final String getLastBuild = "Build: 1.1 - Release date: 17/07/2025";
...
//        }
        ...
        stringbuilder.append("-");
        stringbuilder.append(" ");
        stringbuilder.append(getLastBuild);
...
//        }
    ...
    private void runGameLoop(boolean renderWorldIn) {
...

        {
            ...
            while (accumulatedCalls > 0) {
                EventManager.call(new GameUpdateEvent());
                accumulatedCalls--;
            }
        }
    ...
    private void sendClickBlockToController(boolean leftClick) {
        ...
        if (this.leftClickCounter <= 0 && !player.isHandActive()) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.getType() == net.minecraft.util.math.RayTraceResult.Type.BLOCK) {
        ...
        if (this.leftClickCounter <= 0) {
            if (this.objectMouseOver == null) {
                ...
                switch (this.objectMouseOver.getType()) {
                    case ENTITY:
                        this.playerController.attackEntity(player, ((EntityRayTraceResult) this.objectMouseOver).getEntity());
                        break;
                    case BLOCK:
                        BlockRayTraceResult blockraytraceresult = (BlockRayTraceResult) this.objectMouseOver;
                        BlockPos blockpos = blockraytraceresult.getPos();
    ...
    public void runTick() {
        ...
        if (this.currentScreen == null && this.player != null) {
            if (BaritoneUtils.isActive()) {
                final BiFunction<EventState, TickEventBaritone.Type, TickEventBaritone> tickProvider = TickEventBaritone.createNextProvider();

                for (IBaritone baritone : BaritoneAPI.getProvider().getAllBaritones()) {

                    TickEventBaritone.Type type = baritone.getPlayerContext().player() != null && baritone.getPlayerContext().world() != null
                            ? TickEventBaritone.Type.IN
                            : TickEventBaritone.Type.OUT;
    ...
    public boolean isEntityGlowing(Entity entity) {
        if (entity instanceof PlayerEntity && RenderHook.isPlayerGlowingDisabled()) {
            return false;
        }
