...
{
// leaked by itskekoff; discord.gg/sk3d OOg3ICQ6
    ...
    public static void drawRoundedRect(MatrixStack matrixStack, int x, int y, int width, int height, int radius, int color) {
        ...
        fill(matrix, x + radius, y + radius, x + width - radius, y + height - radius, color);

        // Draw top and bottom rectangles
        fill(matrix, x, y + radius, x + radius, y + height - radius, color);
        fill(matrix, x + width - radius, y + radius, x + width, y + height - radius, color);

        // Draw left and right rectangles
        fill(matrix, x + radius, y, x + width - radius, y + radius, color);
        fill(matrix, x + radius, y + height - radius, x + width - radius, y + height, color);

        // Approximate corners with squares.  This is a very rough approximation.
        fill(matrix, x, y, x + radius, y + radius, color);
        fill(matrix, x + width - radius, y, x + width, y + radius, color);
        fill(matrix, x, y + height - radius, x + radius, y + height, color);
        fill(matrix, x + width - radius, y + height - radius, x + width, y + height, color);
    }
