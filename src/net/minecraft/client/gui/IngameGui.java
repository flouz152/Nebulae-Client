...
// leaked by itskekoff; discord.gg/sk3d nZbCUui3
...
//            System.err.println(score.getScorePoints());
//            System.err.println(score.getScoreScoreboard().getTeamNames());
//            score.getObjective().setDisplayName(new StringTextComponent(TextFormatting.BLUE + "essenceclient.fun"));
                if (itextcomponent1.getString().contains("Ранг")) {
//                System.err.println(itextcomponent1.getString());
//                System.err.println(score.getObjective().func_197890_e().getStyle().setColor(net.minecraft.util.text.Color.fromInt(new Color(255, 255, 255, 255).getRGB())));
                }
//            System.err.println(score.g);
                score.getObjective().setRenderType(ScoreCriteria.RenderType.INTEGER);
...
//                this.getFontRenderer().func_243248_b(p_238447_1_, ITextComponent.getTextComponentOrEmpty("123"), (float) l2, (float) k1 + 125, -1);
            }
        }
    }
    ...
    private void func_238457_e_(MatrixStack p_238457_1_) {
        ...
        if (playerentity != null) {
            ...
            if (j6 == 0) {
                ...
                float saturationLevel = playerentity.getFoodStats().getSaturationLevel();
                if (saturationLevel > 0.0F) {
                    int saturationIcons = Math.min((int) Math.ceil(saturationLevel / 2.0F), 10);
                    
                    for (int sat = 0; sat < saturationIcons; ++sat) {
                        int satX = j1 - sat * 8 - 9;
                        int satY = k1 - 10;

                        this.blit(p_238457_1_, satX, satY, 16, 27, 9, 9);

                        if (sat * 2 + 1 < saturationLevel) {
                            this.blit(p_238457_1_, satX, satY, 52, 27, 9, 9); // Полная золотая иконка
                        } else if (sat * 2 + 1 == saturationLevel) {
                            this.blit(p_238457_1_, satX, satY, 61, 27, 9, 9); // Половина золотой иконки
                        }
                    }
                }
