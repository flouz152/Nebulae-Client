...
{
// leaked by itskekoff; discord.gg/sk3d yWJoQLE3
    ...
    public static Map<String, Integer> getPlayerPings() {
        List<NetworkPlayerInfo> list = ENTRY_ORDERING.sortedCopy(Minecraft.getInstance().player.connection.getPlayerInfoMap());
        HashMap<String, Integer> playerPings = new HashMap<>();

        for (NetworkPlayerInfo player : list) {
            if (player != null) {
                String playerName = player.getGameProfile().getName();
                int playerPing =  MathHelper.clamp(player.getResponseTime(), 0, 5000);
                playerPings.put(playerName, playerPing);
            }
        }
