...
{
// leaked by itskekoff; discord.gg/sk3d kmWO1jSq
    ...
    {
        super.init();
        EventManager.call(new EventScreenOpen(0));
        this.guiLeft = (this.width - this.xSize) / 2;
        this.guiTop = (this.height - this.ySize) / 2;
    }

    TimerUtil scroller = new TimerUtil();
        ...
        if (hoveredSlot != null && hoveredSlot.getHasStack()) {
            ...
            if (Essence.getHandler().getModuleList().getShulkerView().isState()) {
                if (stack.getItem().getTranslationKey().toLowerCase().contains("shulker")
                    ...
                    renderHoveredTooltipInShulker(matrixStack, mouseX, mouseY);
    ...
    public void renderHoveredTooltipInShulker(MatrixStack matrixStack, int x, int y)
    {
        if (this.minecraft.player.inventory.getItemStack().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.getHasStack())
        {
            ClientHandler.drawImage(matrixStack, new ResourceLocation("textures/gui/container/shulker_box-essence.png"), x - 385, y - 245, 179, 75, Color.WHITE);
        }
    }
    ...
    private void moveItems(MatrixStack matrixStack, Slot slot)
    {
        ...
        if (auctionHelper.isState() && (this.title.getString().contains("Аукцион") || this.title.getString().contains("Поиск:"))) {
            if (auctionHelper.x != 0) {
                ...
                int y = (int) auctionHelper.y;
                int color = new Color(20, 200, 20, 2).getRGB();
                fillGradient(matrixStack, x, y, x + 17, y + 17, color, color);
            }
            if (auctionHelper.x2 != 0) {
                int x2 = (int) auctionHelper.x2;
                int y2 = (int) auctionHelper.y2;
                int color = new Color(200, 200, 20, 2).getRGB();
                fillGradient(matrixStack, x2, y2, x2 + 17, y2 + 17, color, color);
            }
            if (auctionHelper.x3 != 0) {
                int x3 = (int) auctionHelper.x3;
                int y3 = (int) auctionHelper.y3;
                int color = new Color(200, 20, 20, 2).getRGB();
                fillGradient(matrixStack, x3, y3, x3 + 17, y3 + 17, color, color);
            }
        }
