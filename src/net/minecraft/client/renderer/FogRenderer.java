...
{
// leaked by itskekoff; discord.gg/sk3d qhCBcGOa
    ...
    public static Color blend(float red, float green, float blue, Color... c) {
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier)
    {
        ...
        else
        {
            ...
            if (customFog.isState()) {
                if(customFog.color.is("Тема")) {
                    ...
                    blend(colors[0] - (customFog.themeBlackness.get()/255), colors[1] - (customFog.themeBlackness.get()/255), colors[2] - (customFog.themeBlackness.get()/255));
                } else if(customFog.color.is("Белый")) {
                    red = 255;
                    green = 255;
                    blue = 255;
                } else if(customFog.color.is("Серый")) {
                    red = 0.5f;    // 128/255 ≈ 0.5
                    green = 0.5f;  // 128/255 ≈ 0.5
                    blue = 0.5f;   // 128/255 ≈ 0.5
                }
            } else {
        ...
        else if (activeRenderInfoIn.getRenderViewEntity() instanceof LivingEntity && ((LivingEntity)activeRenderInfoIn.getRenderViewEntity()).isPotionActive(Effects.Essence_VISION))
        {
            float f7 = GameRenderer.getEssenceVisionBrightness((LivingEntity)activeRenderInfoIn.getRenderViewEntity(), partialTicks);
