...
import beame.util.render.IWrapper;
...
import events.impl.player.EventTrace;
...
// leaked by itskekoff; discord.gg/sk3d qYNi6cxZ
    ...
    public final ActiveRenderInfo activeRender = new ActiveRenderInfo();
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            EventTrace event = (EventTrace) new EventTrace(entity.rotationYaw, entity.rotationPitch);
            EventManager.call(event);

            if (event.isCancel()) {
                vector3d1 = entity.getCustomLook(event.getPitch(), event.getYaw());
            }
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        ...
        if (this.debugView) {
            ...
            if (useFOVSetting) {
                d0 = fieldOfView.isState() ? fieldOfView.fov.get() : this.mc.gameSettings.fov;
    ...
    public void renderSecondHand(MatrixStack p_renderHand_1_, ActiveRenderInfo p_renderHand_2_, float p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_, boolean p_renderHand_6_) {
        if (!this.debugView) {
            Shaders.beginRenderFirstPersonHand(p_renderHand_6_);
            this.resetProjectionMatrix(this.getProjectionMatrix(p_renderHand_2_, p_renderHand_3_, false));
            MatrixStack.Entry matrixstack$entry = p_renderHand_1_.getLast();
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting),
                aspectRatio.isState() ? aspectRatio.aspectRatio.get()
                        : (float) this.mc.getMainWindow().getFramebufferWidth()
                        / (float) this.mc.getMainWindow().getFramebufferHeight(), 0.05f, this.clipDistance));
        return matrixstack.getLast().getMatrix();
    }

    public static float getEssenceVisionBrightness(LivingEntity livingEntityIn, float entitylivingbaseIn) {
        int i = livingEntityIn.getActivePotionEffect(Effects.Essence_VISION).getDuration();
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (renderWorldIn && this.mc.world != null) {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    EventManager.call(new EventRender(partialTicks, matrixstack, mc.getMainWindow(), EventRender.Type.GLASS_HAND, null, activeRender));
                ...
                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        ...
                        if (this.mc.currentScreen instanceof DropDownGui) {
                            ...
                            int i2 = (int) (this.mc.mouseHelper.getMouseX() * (double) this.mc.getMainWindow().getScaledWidth() / (double) this.mc.getMainWindow().getWidth());
                            int j2 = (int) (this.mc.mouseHelper.getMouseY() * (double) this.mc.getMainWindow().getScaledHeight() / (double) this.mc.getMainWindow().getHeight());

                            this.mc.currentScreen.render(matrixstack, i2, j2, this.mc.getTickLength());
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        activerenderinfo.update(this.mc.world, (Entity) (this.mc.getRenderViewEntity() == null ? this.mc.player : this.mc.getRenderViewEntity()), !this.mc.gameSettings.getPointOfView().firstPerson(), this.mc.gameSettings.getPointOfView().thirdPersonFront(), partialTicks);
