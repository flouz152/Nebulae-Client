...
// leaked by itskekoff; discord.gg/sk3d 7sq55Dv1
    ...
    public void rotateTowards(double yaw, double pitch) {
        ...
        Event event = new EventClampPitch();
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                EventMove moveEvent = new EventMove(fromPosition, predictedPosition, pos, onGround, bb, ignoreHorizontal, ignoreVertical, collidedHorizontally, collidedVertically);
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                EventPostMove postMoveEvent = new EventPostMove(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ));
                EventManager.call(postMoveEvent);
            }
    ...
    protected float getSpeedFactor() {
        ...
        if (this instanceof ClientPlayerEntity) EventManager.call(event);
    ...
    public void moveRelative(float friction, Vector3d relative) {
        ...
        EventStrafe event = new EventStrafe(friction, relative, this.rotationYaw);
        if (this instanceof ClientPlayerEntity) {
            EventManager.call(event);
            if (event.isCancel()) {
                return;
    ...
    public double getDistanceEyePos(LivingEntity target) {
        ...
        double y = MathHelper.clamp(this.getPosY() + (double) this.getEyeHeight() - target.getPosY(), 0.0, (double) target.getHeight());
    ...
    public final Vector3d getCustomLook(float pitch, float yaw)
    {
    ...
    public RayTraceResult pick(double rayTraceDistance, float partialTicks, boolean p_213324_4_) {
        ...
        EventTrace event = (EventTrace) new EventTrace(this.rotationYaw, this.rotationPitch);
        EventManager.call(event);

        if (event.isCancel()) {
            vector3d1 = this.getCustomLook(event.getPitch(), event.getYaw());
        }
